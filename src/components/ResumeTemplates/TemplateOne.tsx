...
const TemplateOne = () => {
  ...
  const {
    ...
    error: summaryError,
  } = useSWR("get-summry-prompt/", read);

  const {
    data: experienceData,
    isLoading: experienceLoading,
    error: experienceError,
  } = useSWR("get-experiance-prompt/", read);

  return (
    <div className="relative overflow-hidden border rounded shadow A4 font-montserrat">
      ...
      <div className="relative space-y-8">
        ...
        <div className="grid grid-cols-2 gap-6 px-3">
          <div className="flex gap-1.5">
            <h2 className="vertical-text-rl uppercase tracking-[0.7px] text-[7px] text-end rotate-180">
              WORK EXPERIENCE
            </h2>
              ...
              {workExperience.map(({ header, desc }, index) => (
                <div key={index} className="text-[8px]">
                  ...
                  <ul className="text-[6px] list-inside list-disc pt-1">
                    {experienceData?.["job_experience"]?.map((d, index) => (
                      <li key={index}>{d}</li>
                    ))}
                  </ul>
                </div>
